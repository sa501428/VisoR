...
import com.visor.filters.Filter;
...
public class CameraStreaming {
	...
	public CameraStreaming(int texture,	float[] triangleVertices, float[] textureVertices) 
	{
		...
		int vertexShader = MyNewRenderer.loadShader(GLES20.GL_VERTEX_SHADER, vertexShaderCode);
		int fragmentShader;
		allFilters = FilterVault.getAllFilters();

		for(Filter filter : allFilters){
			fragmentShader = MyNewRenderer.loadShader(GLES20.GL_FRAGMENT_SHADER, filter.getFilterShader());
		...
		fragmentShader = MyNewRenderer.loadShader(GLES20.GL_FRAGMENT_SHADER, FilterVault.inverted);
		...
		fragmentShader = MyNewRenderer.loadShader(GLES20.GL_FRAGMENT_SHADER, FilterVault.fs_GrayCCIR);
	...
	public void draw(float mtx[], DataRelay dataRelay){
		...
		if(dataRelay != null){
			float[] magneticValues = dataRelay.getMagneticValues();
			if(magneticValues != null){
				int targetXHandle = GLES20.glGetUniformLocation(mProgram, "targetX");
				GLES20.glUniform1f(targetXHandle, (float) (0.25*magneticValues[1]+0.5));

				int targetYHandle = GLES20.glGetUniformLocation(mProgram, "targetY");
				GLES20.glUniform1f(targetYHandle, (float) (0.5*magneticValues[2]+0.5));

				//System.out.println(""+magneticValues[0]);
			}
		}
